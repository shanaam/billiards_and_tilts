...
p
```
Note: the outlier is ~ -2 in the Cued Accellerated group.
```{r}
(temp_ANOVA <- aov_car(
  exp_fit_N0 ~ pert * cue + Error(ppid),
  data_per_ppt,
...
p
```
Note: outlier ~2.5 in the Cued Curved group
```{r}
(temp_ANOVA <- aov_car(
  exp_fit_N0 ~ pert * cue + Error(ppid),
  data_per_ppt,
...
```
## Sanity Check: Flick distances
Q: Do participants tend to release the flick early via the trigger? Do flick distances differ across the 4 groups?
```{r}
data_per_ppt <- omnibus_df %>%
  filter(!is.na(flick_dist)) %>%
  group_by(experiment, ppid) %>%
  summarise(
    ppt_mean_dist = mean(flick_dist),
    ppt_ci_dist = vector_confint(flick_dist),
    ppt_sd_dist = sd(flick_dist),
    n = n()
  )
...
data_per_group <- data_per_ppt %>%
  group_by(experiment) %>%
  summarise(
    mean_dist = mean(ppt_mean_dist),
    ci_dist = vector_confint(ppt_mean_dist),
    n = n()
  )

p <- data_per_group %>%
  ggplot(
    aes(x = experiment, y = mean_dist, colour = experiment)
  ) +
  theme_classic() +
  labs(
    x = NULL,
  ...
  geom_hline(
    yintercept = c(0, 0.15), linewidth = 0.4,
    colour = "#CCCCCC", linetype = "solid"
  ) +
  # add data points
  geom_beeswarm(
  ...
  geom_linerange(aes(
    ymin = mean_dist - ci_dist,
    ymax = mean_dist + ci_dist
  ), alpha = 0.5, lwd = 2) +
  geom_point() +
  # set colour palette
  scale_colour_manual(values = pallete_list) +
...
data_per_group <- data_per_ppt %>%
  group_by(experiment) %>%
  summarise(
    mean_dist = mean(ppt_sd_dist),
    ci_dist = vector_confint(ppt_sd_dist),
    n = n()
  )
...
```{r}
(temp_ANOVA <- aov_car(
  ppt_mean_dist ~ experiment + Error(ppid),
  data_per_ppt,
  include_aov = TRUE
))
...
bf <- anovaBF(ppt_mean_dist ~ experiment, data = data.frame(data_per_ppt), progress = FALSE)
print(bf)
print(bayesfactor_inclusion(bf))
```
